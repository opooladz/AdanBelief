...
class AdanBelif(Optimizer):
    ...
    Here we simply combine the two.
    AdaBelif was able to solve classic XOR problem 
    Adan was not. 
    AdanBelif was able to. 
    Maybe there is some benefit on image classifcation. 
    Arguments:
    ...
    def step(self):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                diff_g = grad - state['pre_grad']

                # update = grad + beta2 * diff
               
                exp_avg.mul_(beta1).add_(grad, alpha=1 - beta1)  # m_t
                
                if 'pre_exp_avg' not in state or group['step'] == 1:
                    state['pre_exp_avg'] = exp_avg
                
                diff_exp_avg = exp_avg - state['pre_exp_avg'] 
                    
                copy_exp_avg = exp_avg.clone()
                
                update = grad - exp_avg + beta2 * (diff_g - diff_exp_avg)
                exp_avg_diff.mul_(beta2).add_(diff_g, alpha=1 - beta2)  # diff_t
                exp_avg_sq.mul_(beta3).addcmul_(update, update, value=1 - beta3)  # n_t

                denom = ((exp_avg_sq).add_(group['eps']).sqrt() / math.sqrt(bias_correction3)).add_(group['eps'])
                ...
                state['pre_exp_avg']  = copy_exp_avg
